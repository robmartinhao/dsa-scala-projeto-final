...
class MainActor(config: DataConfig) extends Actor {
  ...
  def receive = {
    case "begin" => {
      ...
      println(s"Numero de Restaurantes: ${restaurants.size}")

      // Armazena todas as localidades no MongoDB
      for (restaurant <- restaurants) { dbPersister ! restaurant }

      // Carrega os dados de sessão
      ...
      // Trata cada arquivo como um batch separado
      for (sf <- sessionFiles) {
        val sessionFile = s"${config.sessionPath}/$sf"
        println(s"Carregando sessoes de: $sessionFile")
        val sessionsData = loadSessionData(sessionFile)
        println(s"\t\tNumero de sessoes gravadas: ${sessionsData.size}")
...
class MessageProducerActor extends Actor {
  ...
  override def preStart() = {
    println(s"=== MessageProducer inicializando: path=${context.self.path} ===")
...
object EntreeDatasetPipeline {
  def main(args: Array[String]) {
    ...
    // Deletando dados existentes do MongoDB
    Database.deleteAll()
    // Limpa a area de transferencia e inicia
    ...
    navigationCount.foreachRDD { rdd =>
      ...
      top5restaurants foreach { entry =>
        println(s"${entry._1} foi recentemente visitado ${entry._2} vezes")
      }
    }

    // Aguarde que todas as conexões estabeleçam serviços diferentes
    Thread.sleep(5 * 1000)

    // Comece o processamento do fluxo e aguarde até que seja informado
    ssc.start()
